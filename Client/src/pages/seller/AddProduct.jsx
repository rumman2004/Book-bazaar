...
const defaultForm = { title: '', author: '', isbn: '', description: '', price: '', condition: 'new', stock_quantity: 1, category_ids: [] };
...
const AddProduct = () => {
  ...
  const toggleCategory = (id) => setForm((value) => ({
    ...value,
    category_ids: value.category_ids.includes(id) ? value.category_ids.filter((item) => item !== id) : [...value.category_ids, id],
  }));
  ...
  const submit = async (event) => {
    ...
    try {
      await SellerService.addBook(payload);
  ...
  if (status === 'loading') return <LoadingState label="Loading product form..." />;
  if (status === 'error') return <ErrorState message={error} />;

  return (
    <section>
      <PageHeader eyebrow="Seller products" title="Add book" description="Create a live book listing in your seller inventory." />
      <form onSubmit={submit} className="rounded-md border border-[var(--color-border)] bg-white p-6 shadow-[var(--shadow-card)]">
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <Input label="Cover image" type="file" accept="image/*" onChange={(event) => setImage(event.target.files?.[0] || null)} className="md:col-span-2" />
        </div>
        <div className="mt-6">
        ...
        <Button type="submit" loading={status === 'saving'} className="mt-6">Create listing</Button>
      </form>
    </section>
  );
};
